...
public class SpigotChatImpl implements SpigotChat {
    ...
    public boolean sendMessage(RpPlayer sender, ChatChannel channel, String message) {
        Player mcSender = ((SpigotRpPlayer) sender).getPlayer().orElse(null);
        ...
        TextComponent msg = new ChatMessage(
            ...
            displayName,
            message,
            ((SpigotChatChannel) channel).getMessageColor(),
            actionMenu,
            mcSender.getName()
        ).toTextComponent();
        ...
        for (RpPlayer p : playerRepo.getPlayers()) {
            ...
            Player mcPlayer = ((SpigotRpPlayer) p).getPlayer().orElse(null);
    ...
    public void setChatChannelAsync(RpPlayer player, ChatChannel channel) {
        configRepo.setChannelAsync(player, channel, r -> {
            ...
            if (err.isPresent()) {
                ...
                ((SpigotRpPlayer) player).getPlayer().ifPresent(p -> {
    ...
    private void loadChannels(ConfigurationSection ch) {
        for (String channelName : ch.getKeys(false)) {
            ...
            channels.put(channelName, new SpigotChatChannel(
                channelName,
                c.getInt("range", 0),
                c.getString("prefix", ""),
                c.getString("permission", null),
                SpigotUtils.getSpigotChatColor(c.getString("messageColor"))
